...
import { ACTIVITY_ID, ZKPASS_MEXC_KYC_SCHEMA_ID } from "./constants";
...
const handleNewMessage: ActivityOnMessage = async (
  ...
  try {
    ...
    toast.success(
      "Congrats, you have completed the activity 'Prove KYC level and claim a MEXC credential'"
    );
  } catch (error: unknown) {
...
const handleExecute: ActivityOnExecute = async (veridaWebUser) => {
  ...
  try {
    ...
    window.open(
      `${config.proof.connectorBaseUrl}?veridaDid=${did}&schemaId=${ZKPASS_MEXC_KYC_SCHEMA_ID}`
    );

    // TODO: Make a localised message of this message
    const message =
      "Please share a MEXC account KYC credential from zkPass protocol";
...
export const activity: Activity = {
  ...
  steps: [
    {
      id: "activities.claimMEXCKYCzkPass.step1",
      defaultMessage:
    ...
    {
      id: "activities.claimMEXCKYCzkPass.step2",
      defaultMessage:
    ...
    {
      id: "activities.claimMEXCKYCzkPass.step3",
      defaultMessage:
