...
import { ACTIVITY_ID, ZKPASS_KUCOIN_KYC_SCHEMA_ID } from "./constants";
...
const handleNewMessage: ActivityOnMessage = async (
  ...
  try {
    ...
    toast.success(
      "Congrats, you have completed the activity 'Prove KYC level and claim a KuCoin credential'"
    );
  } catch (error: unknown) {
...
const handleExecute: ActivityOnExecute = async (veridaWebUser) => {
  ...
  try {
    ...
    window.open(
      `${config.proof.connectorBaseUrl}?veridaDid=${did}&schemaId=${ZKPASS_KUCOIN_KYC_SCHEMA_ID}`
    );

    // TODO: Make a localised message of this message
    const message =
      "Please share a KuCoin account KYC credential from zkPass protocol";
    ...
    return {
      ...
      message: defineMessage({
        id: "activities.claimKuCoinKYCzkPass.executePendingMessage",
        defaultMessage:
    ...
    return {
      status: "todo",
      message: defineMessage({
        id: "activities.claimKuCoinKYCzkPass.gettingExecutionErrorMessage",
...
export const activity: Activity = {
  ...
  points: 50,
  title: defineMessage({
    id: "activities.claimKuCoinKYCzkPass.title",
    defaultMessage: "Prove KYC level and claim a KuCoin credential",
    description:
  ...
  actionLabel: defineMessage({
    ...
    description:
      "Label of the button to start the activity Claim KuCoin KYC level",
  }),
  actionReExecuteLabel: defineMessage({
    id: "activities.claimKuCoinKYCzkPass.actionReExecuteLabel",
    defaultMessage: "Verify again",
    description: "Label of the button to perform the activity again ",
  }),
  actionExecutingLabel: defineMessage({
    id: "activities.claimKuCoinKYCzkPass.actionExecutingLabel",
    defaultMessage: "Sending Request",
    description:
      "Label of the button when the activity 'Claim KuCoin KYC level' is being executed",
  }),
  onInit: handleInit,
  ...
  steps: [
    {
      order: 1,
      description: defineMessage({
        id: "activities.claimKuCoinKYCzkPass.step1.description",
        defaultMessage:
        ...
        description:
          "Step 1 of the activity 'Prove KYC level and claim a KuCoin credential'",
      }),
    },
    {
      order: 2,
      description: defineMessage({
        id: "activities.claimKuCoinKYCzkPass.step2.description",
        defaultMessage:
          "Accept the proof credential received in your inbox to save the credential.",
        description:
          "Step 2 of the activity 'Prove KYC level and claim a KuCoin credential'",
      }),
    },
    {
      order: 3,
      description: defineMessage({
        id: "activities.claimKuCoinKYCzkPass.step3.description",
        defaultMessage:
          "Reply to the message sent by Verida Missions to share your new credential.",
        description:
          "Step 3 of the activity 'Prove KYC level and claim a KuCoin credential'",
      }),
    },
  ],
  resources: [
    {
      label: defineMessage({
        id: "activities.claimKuCoinKYCzkPass.resources.ZkPassPageUrl.label",
