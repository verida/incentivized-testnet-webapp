...
import { type Gamer31TwitchCredentialSubject } from "./types";
...
export function verifyReceivedMessage(
  ...
  const vc =
    data as VeridaVerifiableCredentialRecord<Gamer31TwitchCredentialSubject>;

  // TODO: Consider using zod to validate

  if (
    !vc.credentialSchema ||
    !GAMER31_TWITCH_VC_SCHEMA_URLS.includes(vc.credentialSchema)
  ) {
    return false;
  }
