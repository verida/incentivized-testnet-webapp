...
import { type Gamer31LichessCredentialSubject } from "./types";
...
export function verifyReceivedMessage(
  ...
  const vc =
    data as VeridaVerifiableCredentialRecord<Gamer31LichessCredentialSubject>;

  // TODO: Consider using zod to validate

  if (
    !vc.credentialSchema ||
    !GAMER31_LICHESS_VC_SCHEMA_URLS.includes(vc.credentialSchema)
  ) {
    return false;
  }
